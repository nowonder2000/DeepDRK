...
class DDLK(mdn.MDNModel):
    ...
    def training_step(self, batch, batch_idx, val_step=False):
        ...
        if not val_step:
            ...
            self.log('penalty', lp_xb_tilde.mean(), on_step=True,
                     on_epoch=True, prog_bar=False)

        else:
            self.log('val_loss', loss, on_epoch=True,
                     on_step=True, prog_bar=True)
            self.log('val_penalty', lp_xb_tilde.mean(), on_step=True,
                     on_epoch=True, prog_bar=False)
            logging.info(f"Val Loss: {loss}, Val Penalty: {lp_xb_tilde.mean()}")
    ...
    def on_epoch_end(self):
        ...
        print(f"Epoch: {self.current_epoch}")#, Train Loss: {loss}, Val Loss: {val_loss}, Train Penalty: {penalty}, Val Penalty: {val_penalty}")
