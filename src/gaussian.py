# This .py file is used to compute the Second-order knockoffs
import numpy as np
import cvxpy as cvx
...
class GaussianKnockoffs:
    ...
    def __init__(self, Sigma, method="equi", mu=[], tol=1e-3, cal_Ds=True):
        """
        Constructor
        ...
        if linalg.eigh(self.V, eigvals_only=True, eigvals=(0,0))[0] <= tol:
            ...
            self.LV = np.linalg.cholesky(self.V+1e-8*np.eye(self.p))
