...
def sample_two_moon_high_dim(n_sample=100, n_dim=10):
    ...
    transformer = random_projection.GaussianRandomProjection(n_components=n_dim)
    two_moon_data_high_dim = transformer.fit_transform(two_moon_data)
    ...
    return two_moon_data_high_dim.astype(np.float32)
...
def generateSamples(distType, p, rho_init=0.6):
    '''
    generate a data sampler based on the given distribution
    '''
        
    if distType == "GaussianAR1":
        dataSampler = GaussianAR1(p, 0.5)
        
    elif distType== "GaussianMixtureAR1":
        k = 3
        prop = (6 + (0.5 + np.arange(k) - k / 2)**2)**0.9
        prop = prop / prop.sum()
        rho_list = [rho_init**(i + 0.9) for i in range(k)] #0.6 for V_02, 0.5 for v_03
    #         mu_list=[20 * i for i in range(k)]
        mu_list=[20 * i for i in range(k)]
        dataSampler = GaussianMixtureAR1(p=p, rho_list=rho_list, mu_list=mu_list, proportions=prop)
    elif distType == 'twomoon':
        dataSampler = TwoMoon(p)
    else:
